...
import model2.CmtDeleteOkAction;
import model2.CmtWriteOkAction;
...
public class ControllerEx01 extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			} else if( path.equals( "/board_cmt_ok.do" ) ) {
				boardAction = new CmtWriteOkAction();
				boardAction.execute(request, response);
			
				url = "/WEB-INF/views/model2/board_cmt1_ok.jsp";
				
			} else if( path.equals( "/board_dcmt_ok.do" ) ) {
				boardAction = new CmtDeleteOkAction();
				boardAction.execute(request, response);
			
				url = "/WEB-INF/views/model2/board_dcmt1_ok.jsp";
			}  
